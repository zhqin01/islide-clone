...
public static class WatermarkManager
{
    ...
    public static void Add(string t, float fs, int op, float rot, string ch) { var a = AddInModule.PowerPointApp; if (a?.ActivePresentation == null) return; int r = Convert.ToInt32(ch[0..2], 16), g = Convert.ToInt32(ch[2..4], 16), b = Convert.ToInt32(ch[4..6], 16), n = 0; dynamic pres = a.ActivePresentation; float pw = pres.PageSetup.SlideWidth, ph = pres.PageSetup.SlideHeight; foreach (dynamic s in pres.Slides) { try { dynamic sh = s.Shapes.AddTextbox(1, pw * 0.1f, ph * 0.4f, pw * 0.8f, ph * 0.2f); sh.Name = P + s.SlideIndex; sh.Rotation = rot; sh.Fill.Transparency = 1f - (op / 100f); dynamic tf = sh.TextFrame; tf.TextRange.Text = t; tf.TextRange.Font.Name = "Microsoft YaHei"; tf.TextRange.Font.Size = fs; tf.TextRange.Font.Color.RGB = (b << 16) | (g << 8) | r; tf.TextRange.ParagraphFormat.Alignment = 2; n++; } catch { } } M($"Added watermark to {n} slides."); }
